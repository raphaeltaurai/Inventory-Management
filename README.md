...
I've been working in retail for a while now, and I've always been curious about how inventory moves through our warehouse bins. I wanted to challenge myself to build something practical that could actually help me understand our stock better, so I decided to dive into data analysis using Python. This project has been a great way to sharpen my programming skills while solving a real problem I see at work every day.

The dataset I'm analyzing comes from my workplace - a retail store where I got permission to export transaction and inventory data. The dataset includes two main CSV files: one tracking all inventory transactions (purchases, sales, transfers) and another showing current quantities on hand for each product in different warehouse locations. The data spans several months and includes thousands of transactions across multiple bins and products.

I built this software to answer some questions that have been bugging me: Which bins have the oldest stock sitting in them? How long does inventory typically stay in each location before moving? By tracking inventory aging using FIFO (First In, First Out) principles, I can identify slow-moving stock and potentially help optimize our warehouse layout. Plus, I wanted to see if I could actually implement a proper FIFO aging system from scratch using Pandas. The project now includes automated summary reports that calculate key statistics like average age per bin, inventory distribution by age category, and identification of the slowest-moving products - all things that would be super useful for warehouse managers.
